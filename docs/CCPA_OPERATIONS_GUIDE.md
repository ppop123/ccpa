...
当前本机 `caea69d` 和 50.9 clean candidate 的 strict + build commit 模式均通过；50.9 live 8317 仍健康但尚未切到该 clean candidate。如果未来 50.9 Claude token 再次过期，strict gate 会失败并打印 `provider_hint` 指向 `node dist/index.js --config=... --login --manual`。
...
项目目录 `/Users/wy/auth2api/`，git remote `ccpa` 指向 `https://github.com/ppop123/ccpa.git`（origin 是 fork 上游 AmazingAng/auth2api，平时不推）。当前稳定化分支是 `codex/ccpa-stabilization`；2026-06-22 已验证的本机 live runtime commit 是 `caea69d7c6468880f1d80c54a70bc0be996622b3`，`/health.build.git_dirty=false`。本节后面保留的 `7915477` 记录属于 2026-06-09 历史快照，读者应以第 0 章和 `docs/plans/2026-06-22-ccpa-release-handoff.md` 为当前发布事实源。
...
| 当前 commit | `codex/ccpa-stabilization`，本机 live 已验证 runtime `caea69d7c6468880f1d80c54a70bc0be996622b3` | live 仍是 `/Users/wangyan/ccpa` dirty tree；clean candidate `/Users/wangyan/ccpa-candidates/f3afdf0-20260622165529` 已验证 runtime `caea69d7c6468880f1d80c54a70bc0be996622b3` |
...
下面保留的是 2026-06-09 文档成稿时的历史快照，不再代表 2026-06-22 当前稳定化分支。当前可信发布状态见第 0 章与 `docs/plans/2026-06-22-ccpa-release-handoff.md`；本机 live 已验证 runtime commit `caea69d7c6468880f1d80c54a70bc0be996622b3`，50.9 clean candidate 也在同一 commit 通过 strict build-commit gate。
