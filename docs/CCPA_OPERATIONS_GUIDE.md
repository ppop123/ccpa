...
npm run upstream:matrix -- --codex-model gpt-5.5 --claude-model claude-opus-4-8  # 覆盖计划模型，仍是 dry-run
...
`npm run upstream:matrix -- --apply` 会真实请求本机 CCPA 并消耗 Claude/Codex 订阅额度；只有明确需要真上游验收时再跑。若当前模型 alias 已更新，可加 `--codex-model`、`--claude-model`、`--image-model` 覆盖计划模型；不加 `--apply` 时这些覆盖仍只打印计划，不读取 API key、不调用上游。文本 apply 检查要求 Chat/Responses 实际返回 `ok`，单纯 HTTP 200 或 `status: completed` 不算通过。
...
0. [当前状态覆盖说明（2026-06-22）](#0-当前状态覆盖说明2026-06-22)
1. [总览与架构拓扑](#1-总览与架构拓扑)
...
项目目录 `/Users/wy/auth2api/`，git remote `ccpa` 指向 `https://github.com/ppop123/ccpa.git`（origin 是 fork 上游 AmazingAng/auth2api，平时不推）。当前稳定化分支是 `codex/ccpa-stabilization`；运行态 commit 以 `/health.build.git_commit` 和 `npm run canary -- --require-build-commit "$(git rev-parse HEAD)"` 的实测结果为准，避免文档提交后静态 hash 立刻过期。本节后面保留的 `7915477` 记录属于 2026-06-09 历史快照，读者应以第 0 章和 `docs/plans/2026-06-22-ccpa-release-handoff.md` 为当前发布事实源。
...
### 关键 commit hash（2026-06-09 历史快照）

下面保留的是 2026-06-09 文档成稿时的历史快照，不再代表 2026-06-22 当前稳定化分支。当前可信发布状态见第 0 章与 `docs/plans/2026-06-22-ccpa-release-handoff.md`；每次发布都应从实际 worktree 读取 `git rev-parse HEAD`，再用 strict build-commit gate 验证运行态。

upstream auth2api 仓库在 2026-06-09 **没新 commit**——当时所有 2026-06-09 改动都在 working tree、未 commit。这是当时的历史决策说明，不适用于当前 `codex/ccpa-stabilization` 分支。
...
2. **真上游矩阵验收** — 默认只跑 `npm run upstream:matrix` dry-run；当前模型 alias 和脚本默认不一致时，用 `--codex-model`、`--claude-model`、`--image-model` 覆盖计划。需要花额度确认时，再显式执行 `npm run upstream:matrix -- --apply`，图片另加 `--include-image`；文本检查会要求真实输出 `ok`，避免只用 HTTP shape 误判。
