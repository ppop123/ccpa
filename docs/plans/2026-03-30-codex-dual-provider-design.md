...
将 CCPA 从“单 Claude OAuth 账号代理”扩展为“单实例、单端口、双 provider 代理”，同时向本机脚本暴露：
...
现有 [src/server.ts](../../src/server.ts) 继续保留：
...
Claude 继续使用现有 [src/accounts/manager.ts](../../src/accounts/manager.ts)。
...
3. Claude:
   - 继续走现有 [src/proxy/responses.ts](../../src/proxy/responses.ts) 思路
4. Codex:
...
建议在 [src/config.ts](../../src/config.ts) 中新增：
