...
如果没有任何模型 provider 可用，但显式启用了 Agent Runs，服务仍然可以启动；否则会直接失败。
...
Agent Runs 是一个默认关闭的本地/LAN 自动化接口。调用方把 prompt 和文件内容通过 API
传给 CCPA，CCPA 在 `agents.runs-dir` 下创建临时 workspace，让一个 CLI agent 只在这份
拷贝里工作，然后返回输出、修改文件列表、unified diff 和可下载 artifact。CCPA 不会直接
修改调用方 NAS 或 Mac 上的真实项目目录。
...
P1 支持 `claude-code`、`codex-cli`、`grok-cli`。模式支持 `read-only` 和
`workspace-write`，两者都只作用于临时 workspace。用 `GET /v1/agent-runs/:id`
